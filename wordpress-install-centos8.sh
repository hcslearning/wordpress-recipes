...
WP_URL="https://es.wordpress.org/$WP_TAR"
...
do
    ...
    cd $dir/wordpress
    wp core config --dbhost=localhost --dbname=$BD_NAME --dbuser=$MYSQL_USER --dbpass=$MYSQL_PASSWD --extra-php="define('FS_METHOD', 'direct');"
    ...
    wpcontent=$dir/wordpress/wp-content 
    
    cd $wpcontent
    sudo chown vultr:apache $wpcontent
    sudo chmod 775 $wpcontent
    sudo chown vultr:apache -R plugins themes uploads languages
    sudo chmod 775 -R plugins themes uploads languages
done
